        ...
        <View style={styles.cartView}>
          <View style={styles.cartBar}>
              ...
              <View style={{flex: 1, justifyContent:"center", paddingLeft: px2dp(70)}}>
                {
                  !lens.maxPrice?
                    (<Text style={{color: "#999", fontWeight: "bold"}}>{"请选择或填写充值金额"}</Text>):
                    [<Text key={0} style={{color: "#fff", fontWeight: "bold", fontSize: px2dp(16)}}>{`￥${lens.maxPrice}`}</Text>,
                    <Text key={1} style={{color: "#fff", fontSize: px2dp(10)}}>{""}</Text>]
                }
              </View>
              {!lens.maxPrice?
                <Text style={styles.price}>{"待选择"}</Text>:
                <Text style={[styles.price, {backgroundColor:"#3190e8", color:"#fff"}]}>{"去结算"}</Text>}
